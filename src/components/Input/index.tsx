        ...
        <div className="flex">
          <div className="flex items-center h-5">
            <input
              ...
              type="radio"
              className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300  focus:ring-2 checked:"
              id={label && label}
              {...rest}
            />
          </div>
    ...
    return (
      <fieldset className={`flex flex-col gap-3 ${className} relative`}>
        ...
        <input
          ref={forwardedRef}
          {...register && register(registerName, { required: required })}
          className={`autofill:shadow-[inset_0_0_0px_1000px_rgb(255,255,255)] focus:shadow-none w-full h-10 ${Icon ? `pl-11` : `pl-4`} border ${error ? "border-red-600" : "border-slate-200"} rounded text-sm focus:border-brand-primary-700`}
          type="text"
          placeholder={placeholder && placeholder}
          id={label && label}
          aria-label={label} // Adicionado o atributo aria-label com o valor de label
          {...rest}
        />
